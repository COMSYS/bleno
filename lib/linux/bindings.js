...
blenoBindings.startAdvertising = function(name, serviceUuids) {
  ...
  this._hciBle.startAdvertising({'name': name, 'serviceUuids': serviceUuids});
};

blenoBindings.startAdvertisingWithDict = function(advertisementDict) {
    this._advertising = true;
    
    this._hciBle.startAdvertising(advertisementDict);
};

blenoBindings.setLatency = function(min,max,latency,to_multiplier) {
    debug('setting latency');
    
    this._l2capBle.setLatency(min,max,latency,to_multiplier);
};
