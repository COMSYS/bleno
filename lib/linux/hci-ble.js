...
var HciBle = function() {
    ...
    this._hciBle.stdout.on('error', this.onStdoutError.bind(this));
    this._hciBle.stderr.on('data', this.onStderrData.bind(this));
    
    this._hciBle.on('error', function() { });
    
    this._commSocket = new net.Socket();
    this._commSocket.on('data', this.onSocketData.bind(this));
    this._buffer = "";
    
    this._mtu = 23;
    this._hciHandle = -1;
    this._security = 'unknown';
...
HciBle.prototype.onStdoutError = function(data) {
...
HciBle.prototype.onStdoutData = function(data) {
    ...
    while ((newLineIndex = this._buffer.indexOf('\n')) !== -1) {
        ...
        if ((found = line.match(/^localPort (.*)$/))) {
            var port = parseInt(found[1], 10);
            debug("Connecting to socket on port " + port);
            this._commSocket.connect(port, HOST, function() {debug('socket connected');});
        }
...
var CMD_SET_ADVERTISEMENT_DATA = 0
var CMD_SET_LATENCY = 1
var CMD_DATA = 2
var CMD_DISCONNECT = 3
var CMD_READ_RSSI = 4
var CMD_RSSI = 5
var CMD_DISCONNECTED = 6
var CMD_ACCEPTED = 7
var CMD_HCIHANDLE = 8
var CMD_ADAPTERSTATE = 9
var CMD_SECURITY  =  10
var CMD_L2CAP_DATA = 11

var inputbuffer = new Buffer(4096);
var current_index = 0;

var HEADER_SIZE = 5;

HciBle.prototype.onSocketData = function(inputData) {
    debug('DATA( ' +inputData.length + ' byte): ' + inputData.toString("hex"));
    
    inputData.copy(inputbuffer, current_index);
    
    current_index += inputData.length;
    while(current_index >= HEADER_SIZE) {
        if (current_index < HEADER_SIZE) {
            return;
        }
        var type = inputbuffer.readUInt8(0);
        var len = inputbuffer.readUInt32BE(1);
        if (current_index < HEADER_SIZE+len) {
            return;
        }
        var data = inputbuffer.slice(HEADER_SIZE, HEADER_SIZE+len);
        if(type == CMD_ACCEPTED) {
            clientAddress = data.toString();
            this._mtu = 23; // reset MTU
            this.emit('accept', clientAddress);
        }else if (type == CMD_DISCONNECTED) {
            clientAddress = data.toString();
            this.emit('disconnect', clientAddress);
        }else if (type == CMD_HCIHANDLE) {
            this._hciHandle = data.readUInt16BE(0);
            debug("got hci handle " + this._hciHandle);
        }else if (type == CMD_RSSI) {
            var rssi = data.readUInt8(0);
            this.emit('rssiUpdate', rssi);
        }else if (type == CMD_SECURITY) {
            this._security = data.toString();
        }else if (type == CMD_ADAPTERSTATE) {
            var adapterState = data.toString();
        ...
        }else if (type == CMD_L2CAP_DATA) {
            this.handleRequest(data);
        }
        
        inputbuffer.slice(HEADER_SIZE+len).copy(inputbuffer, 0);
        current_index = current_index - HEADER_SIZE - len;
    }
};
...
HciBle.prototype.startAdvertisingWithOptions = function(advertisementOpts) {
    debug('startAdvertising: options = ' + JSON.stringify(advertisementOpts));
    
    name = advertisementOpts['name'] || "";
    serviceUuids = advertisementOpts['serviceUuids'] || [];
    manufacturerSpecificData = advertisementOpts['manufacturerSpecificData'] || "";
    ...
    if(manufacturerSpecificData.length) {
        advertisementDataLength += 2 + manufacturerSpecificData.length;
    }
    ...
    if(manufacturerSpecificData.length) {
        advertisementData[i++] = 1 + manufacturerSpecificData.length;
        advertisementData[i++] = 0xFF;
        for(j = 0; j < manufacturerSpecificData.length; j++) {
            advertisementData[i++] = manufacturerSpecificData[j];
        }
    }
...
HciBle.prototype.startAdvertisingWithEIRData = function(advertisementData, scanData) {
    ...
    if (advertisementData.length > 31) {
        ...
        var buffer = new Buffer(HEADER_SIZE+2+advertisementData.length+scanData.length);
        buffer.writeUInt8(CMD_SET_ADVERTISEMENT_DATA, 0);
        buffer.writeUInt32BE(2+advertisementData.length+scanData.length, 1);
        buffer.writeUInt8(advertisementData.length, HEADER_SIZE);
        buffer.writeUInt8(scanData.length, HEADER_SIZE+1);
        advertisementData.copy(buffer, HEADER_SIZE+2);
        scanData.copy(buffer, HEADER_SIZE+2+advertisementData.length);
        this._commSocket.write(buffer);
        //this._hciBle.stdin.write("advertise"+advertisementData.toString('hex') + ' ' + scanData.toString('hex') + '\n');
    }
...
HciBle.prototype.kill = function(sig) {
    this._commSocket.destroy()
    this._hciBle.kill(sig);
};

HciBle.prototype.setLatency = function(min,max,latency,to_multiplier) {
    
    var buf =  new Buffer(10);
    buf.writeUInt16BE(this._hciHandle, 0);
    buf.writeUInt16BE(min,2);
    buf.writeUInt16BE(max,4);
    buf.writeUInt16BE(latency,6);
    buf.writeUInt16BE(to_multiplier,8);
    
    var buffer = new Buffer(HEADER_SIZE+buf.length);
    buffer.writeUInt8(CMD_SET_LATENCY, 0);
    buffer.writeUInt32BE(buf.length, 1);
    buf.copy(buffer, HEADER_SIZE);

    this._commSocket.write(buffer);
    
    //this._hciBle.stdin.write("latency" + buf.toString("hex") + '\n');
};


HciBle.prototype.setServices = function(services) {
    ...
    for (i = 0; i < allServices.length; i++) {
        ...
        for (j = 0; j < service.characteristics.length; j++) {
            ...
            if (properties & 0x10) {
                ...
                secure: (secure & 0x10) ? (0x02 | 0x08) : 0,
...
HciBle.prototype.disconnect = function() {
    debug('disconnect request');
    var buffer = new Buffer(HEADER_SIZE);
    buffer.writeUInt8(CMD_DISCONNECT, 0);
    buffer.writeUInt32BE(0, 1);
    
    this._commSocket.write(buffer);
    //this._hciBle.stdin.write("disconnect\n");
};

HciBle.prototype.updateRssi = function() {
    var buffer = new Buffer(HEADER_SIZE);
    buffer.writeUInt8(CMD_READ_RSSI, 0);
    buffer.writeUInt32BE(0, 1);
    
    this._commSocket.write(buffer);
    //this._hciBle.stdin.write("readrssi\n");
};

HciBle.prototype.send = function(data) {
    debug('send: ' + data.toString('hex'));
    var buffer = new Buffer(HEADER_SIZE+data.length);
    buffer.writeUInt8(CMD_DATA, 0);
...
HciBle.prototype.errorResponse = function(opcode, handle, status) {
...
HciBle.prototype.handleRequest = function(request) {
...
HciBle.prototype.handleMtuRequest = function(request) {
...
HciBle.prototype.handleFindInfoRequest = function(request) {
    var response = null;
...
HciBle.prototype.handleFindByTypeRequest = function(request) {
    var response = null;
...
HciBle.prototype.handleReadByGroupRequest = function(request) {
    var response = null;
...
HciBle.prototype.handleReadByTypeRequest = function(request) {
    var response = null;
...
HciBle.prototype.handleReadOrReadBlobRequest = function(request) {
    var response = null;
    ...
    if (handle) {
        ...
        if (handleType === 'service' || handleType === 'includedService') {
            ...
            if (handleProperties & 0x02) {
                if (handleSecure & 0x02 && (this._security !== 'medium' && this._security !== 'high')) {
...
HciBle.prototype.handleWriteRequestOrCommand = function(request) {
    var response = null;
    ...
    if (handle) {
        ...
        if (handleProperties && (withoutResponse ? (handleProperties & 0x04) : (handleProperties & 0x08))) {
            ...
            if (handleSecure & (withoutResponse ? 0x04 : 0x08) && (this._security !== 'medium' && this._security !== 'high')) {
