...
HciBle.prototype.onStdoutData = function(data) {
    ...
    while ((newLineIndex = this._buffer.indexOf('\n')) !== -1) {
        ...
        }else if ((found = line.match(/^l2cap_accept (.*)$/))) {
            clientAddress = found[1];
            
            this._mtu = 23; // reset MTU
            
            this.emit('accept', clientAddress);
        }else if ((found = line.match(/^l2cap_hciHandle (.*)$/))) {
            this._hciHandle = parseInt(found[1], 10);
            
        } else if ((found = line.match(/^l2cap_disconnect (.*)$/))) {
            clientAddress = found[1];
            
            this.emit('disconnect', clientAddress);
        } else if ((found = line.match(/^l2cap_rssi = (.*)$/))) {
            var rssi = parseInt(found[1], 10);
            
            this.emit('rssiUpdate', rssi);
        } else if ((found = line.match(/^l2cap_security (.*)$/))) {
            this._security = found[1];
        } else if ((found = line.match(/^l2cap_data (.*)$/))) {
...
HciBle.prototype.setLatency = function(min,max,latency,to_multiplier) {
    ...
    buf.writeUInt16LE(this._hciHandle, 0);
    buf.writeUInt16LE(min,2);
    buf.writeUInt16LE(max,4);
    buf.writeUInt16LE(latency,6);
    buf.writeUInt16LE(to_multiplier,8);
...
HciBle.prototype.send = function(data) {
    ...
    hexedData = data.toString('hex');
    var buffer = new Buffer(4);
    buffer.writeUInt32LE(hexedData.length, 0);
    this._hciBle.stdin.write("data" + buffer.toString('hex') + hexedData+"\n");
};
