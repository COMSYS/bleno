...
Bleno.prototype.startAdvertisingWithOptions = function(advertisingOpts, callback) {
    if (callback) {
        this.once('advertisingStart', callback);
    }
    
    serviceUuids = advertisingOpts['serviceUuids'];
    ...
    advertisingOpts['serviceUuids'] = undashedServiceUuids;
    
    this._bindings.startAdvertisingWithOptions(advertisingOpts);
};


Bleno.prototype.startAdvertising = function(name, serviceUuids, callback) {
  advertisingOpts = {'name': name, 'serviceUuids': undashedServiceUuids};
    
  this.startAdvertisingWithOptions(advertisingOpts, callback);
};
...
if (platform === 'linux') {
    // Linux only API
    Bleno.prototype.setLatency = function(min,max,latency,to_multiplier) {
        this._bindings.setLatency(min, max, latency, to_multiplier);
    };
}
